...
for init in ${sequence[@]};do
  ...
  echo "" >> exe_${jobName}.sh
    echo "./bin/run_pulls.exe --input ../xmlAnaWSBuilder/run/WSAllCats/vbf_cp_m00/vbf_cp_m00.root --workspace combWS --data asimovData_SB_SM --poi mu_VBF_RW[0:5] --fix mu_VBF_SM[0],mu[1] --parameter ${sysList[${num}]}" >> exe_${jobName}.sh
  done
